...
public class FileEx02ReadJava {
	...
	/**
	 * *.java파일 내에서 keyword가 몇 개 있는지 count
	 * @param dir
	 * @param keyword
	 * @throws IOException
	 */
	public static void findInfile(File dir, String keyword)throws IOException{
		...
		for(int i = 0; i < files.length; i++) {
			...
			//dir, file
			if(files[i].isDirectory() == true) {//디렉토리이면 재귀호출
				findInfile(files[i], keyword);
			}else {//파일이면서 *.java
